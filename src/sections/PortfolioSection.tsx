...
const projects = [
  { id: 1, title: 'Sistem ERP UMKM', category: 'Web App', desc: 'Sistem manajemen bisnis terintegrasi untuk UMKM dengan modul keuangan, inventory, CRM, dan laporan otomatis.', tech: ['React', 'Node.js', 'PostgreSQL'], icon: Code2, color: 'from-[#F4B942] to-[#D4941A]', colorBright: 'from-[#FFD166] to-[#F4B942]' },
  ...
  { id: 3, title: 'E-Commerce Pro', category: 'Web App', desc: 'Toko online dengan payment gateway multi-bank, manajemen produk, dan sistem review pelanggan.', tech: ['Vue.js', 'Laravel', 'MySQL'], icon: ShoppingCart, color: 'from-[#D4941A] to-[#F4B942]', colorBright: 'from-[#F4B942] to-[#FFD166]' },
...
export default function PortfolioSection() {
  ...
  return (
    <section id="portofolio" className="relative py-24 lg:py-32 overflow-hidden" style={{ background: 'linear-gradient(180deg, #FFFBF3 0%, #FFF8E7 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            ...
            >
              Proyek <span style={{ color: '#D4941A' }}>Terbaru</span>
            </motion.h2>
            <motion.div
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((project, i) => (
              <motion.div
                ...
                className="group relative rounded-2xl overflow-hidden cursor-pointer bg-white border border-[#F0E6D3] hover:border-[#F4B942]/40 transition-colors duration-300 shadow-sm hover:shadow-xl hover:shadow-[#F4B942]/10"
              >
                {/* Gradient Top Half */}
                <div className="relative h-52 overflow-hidden">
                  ...
                  <motion.div
                    className="absolute inset-0 bg-[#2D2D3A]/60 backdrop-blur-sm flex items-center justify-center"
                    initial={{ opacity: 0 }}
                    whileHover={{ opacity: 1 }}
                    transition={{ duration: 0.3 }}
                  >
                    <motion.div
                      className="flex items-center gap-2 px-5 py-2.5 bg-white rounded-full text-[#2D2D3A] text-sm font-semibold shadow-lg"
                ...
                {/* White Bottom Half */}
                <div className="p-6">
                  <h3 className="text-[#2D2D3A] font-bold text-lg mb-2 group-hover:text-[#D4941A] transition-colors duration-300">
                    {project.title}
                  </h3>
                  <p className="text-[#6B6B7B] text-sm mb-4 line-clamp-2 leading-relaxed">
                    {project.desc}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((t) => (
                      <motion.span
                        ...
                        className={`px-3 py-1 bg-gradient-to-r ${techBadgeColors[t] || 'from-[#F4B942] to-[#D4941A]'} text-white text-[10px] font-semibold rounded-full shadow-sm`}
                      >
                        {t}
                      </motion.span>
                    ))}
      ...
      <AnimatePresence>
        {selectedProject && (
          ...
          >
            {/* Backdrop with blur */}
            <motion.div
              className="absolute inset-0 bg-[#2D2D3A]/50 backdrop-blur-sm"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            />
            ...
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-lg w-full overflow-hidden border border-[#F0E6D3] shadow-2xl relative z-10"
            ...
            >
              ...
              <div className="p-8">
                <motion.h3
                  className="text-[#2D2D3A] font-bold text-2xl mb-3"
                ...
                <motion.p
                  className="text-[#6B6B7B] text-sm mb-6 leading-relaxed"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <p className="text-[#2D2D3A] text-xs font-semibold uppercase tracking-wider mb-3">Tech Stack</p>
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.tech.map((t, i) => (
                      <motion.span
                        ...
                        className={`px-4 py-1.5 bg-gradient-to-r ${techBadgeColors[t] || 'from-[#F4B942] to-[#D4941A]'} text-white text-xs font-semibold rounded-full shadow-md`}
                      >
                        {t}
                      </motion.span>
                    ))}
