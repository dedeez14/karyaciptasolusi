...
export default function CareerSection() {
  return (
    <section id="karir" className="relative py-24 lg:py-32 overflow-hidden" style={{ background: 'linear-gradient(180deg, #FFFBF3 0%, #FFFFFF 50%, #FFFBF3 100%)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: easing }} className="text-center mb-14">
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-[#2D2D3A] mb-3" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
            Bergabung <span style={{ color: '#D4941A' }}>Bersama Kami</span>
          </h2>
          <p className="text-[#6B6B7B] max-w-lg mx-auto text-sm">Kami selalu mencari talenta terbaik untuk memperkuat tim. Lihat lowongan yang tersedia.</p>
        </motion.div>
          ...
          {jobs.map((job, i) => (
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <h3 className="text-lg font-bold text-[#2D2D3A]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{job.title}</h3>
                  ...
                  <p className="text-sm text-[#6B6B7B] mb-3">{job.desc}</p>
                  <div className="flex flex-wrap gap-3">
                    <span className="flex items-center gap-1 text-xs text-[#6B6B7B]"><MapPin size={12} style={{ color: '#D4941A' }} /> {job.location}</span>
                    <span className="flex items-center gap-1 text-xs text-[#6B6B7B]"><Banknote size={12} style={{ color: '#D4941A' }} /> {job.salary}</span>
                    <span className="flex items-center gap-1 text-xs text-[#6B6B7B]"><GraduationCap size={12} style={{ color: '#D4941A' }} /> {job.edu}</span>
                  </div>
                </div>
                <a href="https://wa.me/6285601771312?text=Halo,%20saya%20tertarik%20melamar%20posisi%20" target="_blank" rel="noopener noreferrer"
              ...
              <div className="mt-4 pt-4 border-t border-[#F0E6D3]">
                <p className="text-xs font-semibold text-[#6B6B7B] mb-2">Kualifikasi:</p>
                <div className="grid sm:grid-cols-2 gap-1.5">
                  {job.requirements.map((r) => (
                    <span key={r} className="flex items-start gap-1.5 text-xs text-[#2D2D3A]/70">
                      <Briefcase size={12} className="text-[#F4B942] mt-0.5 flex-shrink-0" /> {r}
                    </span>
                  ))}
                </div>
              </div>
